            ...
            if (length >= firstColumn)
            {
                ...
                if (Editor.SyntaxHighlightingMode != SyntaxHighlightingModes.None && syntaxTree != null)
                {
                    ...
                    foreach ((int start, int end, Color color) range in Consolidate(colorRanges, start, end))
                    {
                        ...
                        Avalonia.Media.FormattedText formattedText = new Avalonia.Media.FormattedText(this.Editor.Text.ToString(new TextSpan(range.start, range.end - range.start)), CultureInfo.InvariantCulture, FlowDirection.LeftToRight, this.Editor.Typeface, this.Editor.FontSize, new SolidColorBrush(range.color));
                ...
                else
                {
                    ...
                    Avalonia.Media.FormattedText formattedText = new Avalonia.Media.FormattedText(text, CultureInfo.InvariantCulture, FlowDirection.LeftToRight, this.Editor.Typeface, this.Editor.FontSize, Brushes.Black);
